...
(function(){
  var footHTML = '<b>About this data:</b> The DOJ National Caseload data dashboard is sourced and cleaned from monthly filings from the Department of Justice’s Legal Information Office Network System (LIONS), <a href="https://www.justice.gov/usao/media/1443941/dl?inline" target="_blank">also known as the DOJ Case Management System (CMS)</a>. This dashboard allows users to view and sort data from 1994 to 2026. Note that not all DOJ cases are reflected in this data set because the system is used by U.S. Attorneys Offices (USAOs), meaning it may not include cases from the department’s litigating components, like the Criminal or National Security Divisions.</p><p>Justice Connection does not manipulate the data or conduct any analysis of the data. Data validation is possible by comparing the DOJ’s annual reports to monthly data downloads. However, given the manual input processes of the LIONS system and lack of information due to redaction or sealed cases, all data cannot be independently validated.</p><p>Monthly updates include revised data from previous months, meaning updates may differ from previously posted numbers. Based on data analysis, Justice Connection estimates that a near-complete picture of monthly filings can be assumed after a three-month lag, which we categorize as "settled." For the most accurate monthly filing data, users should avoid drawing conclusions based on the last three months, because more case data will likely be added to those months. Data in this unfinished period is labeled as "provisional." Terminations generally take more time  to settle - as much as six months - so the provisional marking on termination and disposition series covers a wider window.</p><p>Program categories are sorted by “primary” based on the first-entered code. For more details on program definitions and the legal statutes they include, <a href="https://www.justice.gov/usao/file/787346/dl?inline" target="_blank">please refer to the DOJ’s LIONS Code List here.</a></p><p><a href="reading-the-data.html">More information on data documentation can be found here</a>.</p>';
